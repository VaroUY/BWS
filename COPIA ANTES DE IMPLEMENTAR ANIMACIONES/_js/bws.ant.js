...
socket.on('abrirSeleccionPersonaje', (jugadoresServer) => {
    ...
    // AJUSTE DE DIMENSIONES DEL MODAL (30% menos de ancho)
    const modalInterno = modal.querySelector('div');
    if (modalInterno) {
        modalInterno.style.width = "280px"; 
        modalInterno.style.padding = "25px"; // Reducimos padding para ganar altura
    }
    ...
    jugadoresServer.forEach((jug, index) => {
        if (jug.nombre !== "Sin Asignar") {
            ...
            const colorTextoDisponible = "#000000"; 
            const colorTextoOcupado = "#00BFFF";    
            const rojoSuave = "#a52a3a"; 

            let colorActual = (jug.socketId !== null) ? colorTextoOcupado : colorTextoDisponible;

            // Inyectamos el HTML con el color FORZADO
            btn.innerHTML = `
                <img src="_imagenes/IconoUser.png" style="width: 20px; height: 20px; margin-right: 12px; border:none;">
                <span class="nombre-jugador" style="flex-grow: 1; text-align: left; color: ${colorActual} !important; font-size: 14px;">
                    ${jug.nombre} ${jug.socketId !== null ? ' (Ocupado)' : ''}
                </span>
            `;
            
            // Estilos del botón (Reducidos en un 20% aprox)
            btn.style.display = "flex";
            btn.style.alignItems = "center";
            btn.style.width = "100%";
            btn.style.padding = "8px 15px"; // Menos padding para achicar altura
            ...
            btn.style.marginBottom = "8px"; // Espacio entre botones

            if (jug.socketId !== null) {
                ...
                btn.onmouseover = () => { 
                    btn.style.background = rojoSuave; 
                    btn.querySelector('.nombre-jugador').style.setProperty('color', '#ffffff', 'important');
                };
                btn.onmouseout = () => { 
                    btn.style.background = "#ffffff"; 
                    btn.querySelector('.nombre-jugador').style.setProperty('color', colorTextoDisponible, 'important');
                };
...
socket.on('partidaListaParaEmpezar', (estadoRecibido) => {
    ...
    document.getElementById("Jugador1_Linea").style = "border-style: solid; border-color: red; border-width: 0.5px;";
    document.getElementById("Jugador1_V_Linea").style = "border-style: solid; border-color: red; border-width: 0.5px;";
...
socket.on('actualizarTurno', (data) => {
    ...
    document.getElementById("Jugador" + data.turno + "_Linea").style = "border-style: solid; border-color: red; border-width: 0.5px;";
    document.getElementById("Jugador" + data.turno + "_V_Linea").style = "border-style: solid; border-color: red; border-width: 0.5px;";
...
function jugarCartasCompletar(_esDelMaso, _queCarta) {
    ...
    hacerDraggable(cajaBlanca, mangoTitulo);
}
...
function dibujoGraficaBarras(_Heineken, _Gatorade, _Nike, _McDonalds) {
    ...
    // Estos valores aseguran que el dibujo interno coincida con el tamaño del contenedor
    myCanvas.width = 390;
    myCanvas.height = 550;
    ...
    function animarFrame(tiempoActual) {
        ...
        for (let marca of marcas) {
            ...
            const x = padding + (index * (anchoBarra + gap)) + (gap / 2);
            const y = myCanvas.height - padding - altoBarra;

            let gradient = ctx.createLinearGradient(x, y, x, myCanvas.height - padding);
            ...
            ctx.fillText(Math.round(valorCalculado), x + (anchoBarra / 2), y - 10);
...
function sincronizarMiPantallaAlServidor() {
    ...
    var _miIndice = miAsientoLocal - 1;
    for (var i = 0; i < 4; i++) {
        var jugador = { nombre: _jugadores[i][0] };
        // Solo enviamos datos economicos del jugador local
        if (i === _miIndice) {
            jugador.cash = Number(format2Num(document.getElementById("Jugador"+String(i+1)+"_Cash").innerHTML));
            jugador.h    = Number(format2Num(document.getElementById("Jugador"+String(i+1)+"_Heineken").innerHTML));
            jugador.g    = Number(format2Num(document.getElementById("Jugador"+String(i+1)+"_Gatorade").innerHTML));
            jugador.n    = Number(format2Num(document.getElementById("Jugador"+String(i+1)+"_Nike").innerHTML));
            jugador.m    = Number(format2Num(document.getElementById("Jugador"+String(i+1)+"_McDonalds").innerHTML));
            jugador.c1   = _jugadores[i][1];
            jugador.c2   = _jugadores[i][2];
            jugador.c3   = _jugadores[i][3];
            jugador.c4   = _jugadores[i][4];
        }
...
function abrirHistorial() {
    ...
    if (_historialTransacciones.length === 0) {
        contenido.innerHTML = '<p style="color:#888; text-align:center; margin-top:30px; font-size:15px;">Sin transacciones aun.</p>';
        document.getElementById('ModalHistorial').style.display = 'flex';
        return;
    }

    _historialTransacciones.forEach(function(item) {
        if (item.tipo === 'separador') {
            contenido.innerHTML += '<hr style="border:none; border-top:2px dashed #ccc; margin:10px 0;">';
            ...
            contenido.innerHTML += '<p style="margin:5px 0; padding:8px 14px; background:#ffffff; border-left:4px solid #89152b; border-radius:4px; color:#111; font-size:14px;"><b style="color:#89152b;">' + item.jugador + '</b> | CARTA [' + item.origen + '] → ' + descripcion + '</p>';
        } else {
            var colorBorde = item.tipo === 'Compra' ? '#1a7a3c' : '#b84c00';
            contenido.innerHTML += '<p style="margin:5px 0; padding:8px 14px; background:#ffffff; border-left:4px solid ' + colorBorde + '; border-radius:4px; color:#111; font-size:14px;"><b style="color:#89152b;">' + item.jugador + '</b> | <b style="color:' + colorBorde + ';">' + item.tipo + '</b> → ' + item.empresa + ' <b style="color:#111;">' + item.cantidad + '</b> acciones</p>';
        }
    });
