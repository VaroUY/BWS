        ...
        if (jug.nombre !== "Sin Asignar") {
            ...
            btn.innerHTML = `
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" style="margin-right:10px; flex-shrink:0;">
                    <circle cx="12" cy="8" r="4" fill="${ocupado ? '#3a4a5a' : '#ffc107'}"/>
                    <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" stroke="${ocupado ? '#3a4a5a' : '#ffc107'}" stroke-width="2" stroke-linecap="round"/>
                </svg>
                <span class="nombre-jugador" style="flex-grow: 1; text-align: left; font-family: 'Rajdhani', sans-serif; font-size: 14px; font-weight: 700; letter-spacing: 2px; color: ${ocupado ? '#3a4a5a' : '#c8d8e8'};">
                    ${jug.nombre.toUpperCase()} ${ocupado ? '<span style="color:#3a4a5a; font-size:11px; letter-spacing:1px;">(OCUPADO)</span>' : ''}
                </span>
            `;
            ...
            btn.style.border = ocupado ? "1px solid #1a2535" : "1px solid #1e3a5f";
            btn.style.borderRadius = "8px";
            btn.style.transition = "all 0.2s ease";
            btn.style.background = ocupado ? "#080c10" : "#0d1929";
            btn.style.cursor = ocupado ? "not-allowed" : "pointer";
            if (ocupado) {
                btn.disabled = true;
            } else {
                btn.onmouseover = () => {
                    ...
                    btn.querySelector('.nombre-jugador').style.color = "#ffffff";
                };
                btn.onmouseout = () => {
                    btn.style.background = "#0d1929";
                    btn.style.borderColor = "#1e3a5f";
                    btn.querySelector('.nombre-jugador').style.color = "#c8d8e8";
                };
...
socket.on('actualizarCliente', (estadoJuegoServidor) => {
    ...
    estadoJuegoServidor.jugadores.forEach((jugadorServer, index) => {
        // Solo ejecutamos la lógica si el nombre es distinto a "Sin Asignar"
        if (jugadorServer.nombre !== "Sin Asignar") {
            ...
            console.log(jugadorServer);
        }
        // Si es "Sin Asignar", el bucle simplemente no hace nada y pasa al siguiente índice
    });
...
//        finalizarJugada();
    ...
    document.getElementById('ModalCartaImagen').src = document.getElementById('CartaJugada').src;
    modal.style.display = "flex"; 
